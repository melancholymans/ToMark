...
func main() {
	...
	setDB := func() *sql.DB {
		con, er := sql.Open("sqlite3", "data.db3")
		if err(er) {
			return nil
		}
		return con
	...
	// find data function.
	ff := func() {
		var qry string = "select * from md_dat where title like ?"
		con := setDB()
		if con == nil {
			return
		}
	...
	// find by id function.
	idf := func(id int) {
		var qry string = "select * from md_dat where id = ?"
		con := setDB()
		if con == nil {
			return
		}
	...
	sf := func() {
		dialog.ShowConfirm("Alert", "Save data?", func(f bool) {
			if f {
				...
				qry := "insert into md_dat (title,url,markdown) values (?,?,?)"
